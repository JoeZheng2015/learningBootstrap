...
+function ($) {
  ...
  /** 通过找到当前item父类下的所有item，然后用$els.index(el)来定位当前item的index */
  Carousel.prototype.getItemIndex = function (item) {
    this.$items = item.parent().children('.item')
    return this.$items.index(item || this.$active)
  }
  /**
   * 根据方向和当前激活的item，返回下一个item
   * @param  {string} direction 轮播的方向
   * @param  {object} active    当前激活的item对象
   * @return {object}           下一个item
   */
  Carousel.prototype.getItemForDirection = function (direction, active) {
    var activeIndex = this.getItemIndex(active)
    // 判断是否wrap，即第一个往前翻或最后一个往后翻
  ...
  var clickHandler = function (e) {
    ...
    // 点击左右箭头时，options.slide为next|prev，这样会在Plugin中调用对应的data[action]（如data['next']），完成相应动作
    Plugin.call($target, options)
    // 若是点击bullet，则直接调用to方法到指定位置
    if (slideIndex) {
